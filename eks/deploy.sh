...
fix_yarn_key_if_needed() {
    # only attempt if any apt source mentions dl.yarnpkg.com
    if grep -R --quiet "dl.yarnpkg.com" /etc/apt 2>/dev/null; then
        # if keyring already present and yarn.list already signed-by, nothing to do
        if [ -f /usr/share/keyrings/yarn-archive-keyring.gpg ]; then
            return 0
        fi

        echo "Detected Yarn APT source but missing keyring; attempting to add keyring (non-fatal)..."
        ...
        if curl -fsSL https://dl.yarnpkg.com/debian/pubkey.gpg | sudo gpg --dearmor -o /usr/share/keyrings/yarn-archive-keyring.gpg; then
            echo "Yarn keyring added to /usr/share/keyrings/yarn-archive-keyring.gpg."
            # ensure the yarn list uses signed-by (overwrite or create)
            echo "deb [signed-by=/usr/share/keyrings/yarn-archive-keyring.gpg] https://dl.yarnpkg.com/debian stable main" \
                | sudo tee /etc/apt/sources.list.d/yarn.list > /dev/null
        else
            echo "Warning: failed to fetch/install Yarn GPG key; continuing without fixing (non-fatal)."
        fi
    fi
    return 0
}
